...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { imageUrl, context, lengthPreference = "medium", targetWordCount = 40 } = body

    if (!imageUrl) {
      return NextResponse.json({ 
        error: "Image is required. Please upload an image or provide a valid image URL." 
      }, { status: 400 })
    }
    ...
    if (!isBase64 && !isHttpUrl) {
      return NextResponse.json({ 
        error: "Invalid image format. Please provide a valid image URL or upload an image file." 
      }, { status: 400 })
    }
    ...
    if (user) {
      ...
      if (!hasCredits) {
        return NextResponse.json({ 
          error: "Insufficient credits. You need 1 credit to generate alt text.",
...
- Aim for approximately ${wordCount} words in your response`
    ...
    let userPrompt = "Please generate concise, descriptive alt text for this image."
    ...
    const calculateMaxTokens = (wordCount: number) => {
      const baseTokens = Math.max(50, Math.min(1000, Math.ceil(wordCount * 1.5)))
      return baseTokens
    }
    ...
    if (cleanedAltText.length > maxCharacters) {
      ...
      const finalAltText = cutPoint > 50 ? truncated.substring(0, cutPoint + 1) : truncated
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key')) {
        return NextResponse.json({ 
          error: "Service configuration error. Our team has been notified. Please try again in a few minutes." 
        }, { status: 500 })
      }
      ...
      if (error.message.includes('content filter') || error.message.includes('safety')) {
        return NextResponse.json({ 
          error: "This image cannot be processed due to content restrictions. Please try a different image." 
        }, { status: 400 })
      }

      if (error.message.includes('image') && error.message.includes('format')) {
        return NextResponse.json({ 
          error: "Unsupported image format. Please use JPG, PNG, GIF, or WebP format." 
        }, { status: 400 })
      }
      ...
      if (error.message.includes('too short') || error.message.includes('empty')) {
        errorLogger.logMinorError("Alt text generation produced invalid output", {
          component: "generate-alt-text-api",
          userId: user?.id,
          error: error.message
        })
        return NextResponse.json({ 
          error: "Failed to generate meaningful alt text. Please try with a different image or add more context." 
        }, { status: 500 })
      }
    }
    ...
    return NextResponse.json({ 
      error: "Failed to generate alt text. Please check your image and try again." 
    }, { status: 500 })
  }
}
