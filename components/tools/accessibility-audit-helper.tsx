...
import { hasUnlimitedAccess, getUnlimitedAccessRemainingTime, formatRemainingTime } from '@/lib/unlimited-access'
...
const TECH_STACKS = [
  ...
  'HTML/CSS/JS', 'jQuery', 'WordPress', 'Shopify', 'Other'
]

const COMPONENT_TYPES = [
  'Navigation', 'Forms', 'Buttons', 'Modals/Dialogs', 'Dropdowns',
  'Tables', 'Images', 'Videos', 'Carousels', 'Accordions', 'Tabs', 'Other'
]
...
export default function AccessibilityAuditHelper() {
  ...
  const { getTrialStatus, updateTrialStatus, fetchTrialStatus } = useTrialStatus()
  ...
  const handleAnalyze = async (e: React.FormEvent) => {
    ...
    try {
              ...
              switch (data.type) {
                ...
                case 'partial':
                  setStreamingContent(prev => {
                    ...
                    const detectCurrentSection = (content: string) => {
                      ...
                      const fieldPatterns = [
                        { field: 'issueTitle', label: 'Issue Identification', pattern: /"issueTitle":\s*"([^"]*)"?$/ },
                        { field: 'severity', label: 'Severity Assessment', pattern: /"severity":\s*"([^"]*)"?$/ },
                        { field: 'actualResult', label: 'Problem Analysis', pattern: /"actualResult":\s*"([^"]*)"?$/ },
                        { field: 'expectedResult', label: 'Solution Planning', pattern: /"expectedResult":\s*"([^"]*)"?$/ },
                        { field: 'userImpact', label: 'Impact Assessment', pattern: /"userImpact":\s*"([^"]*)"?$/ },
                        { field: 'recommendations', label: 'Expert Recommendations', pattern: /"recommendations":\s*"([^"]*)"?$/ },
                        { field: 'codeExample', label: 'Code Solution', pattern: /"codeExample":\s*"([^"]*)"?$/ },
                      ]
                      
                      // Check for WCAG criteria being written
                      if (content.includes('"wcagCriteria"') && content.includes('[')) {
                        const wcagMatch = content.match(/"wcagCriteria":\s*\[([\s\S]*?)$/)
                        if (wcagMatch) {
                          return { section: 'WCAG Compliance Analysis', text: 'Analyzing WCAG criteria and compliance requirements...' }
                        }
                      }
                      
                      // Check for implementation steps
                      if (content.includes('"implementationSteps"') && content.includes('[')) {
                        const stepsMatch = content.match(/"implementationSteps":\s*\[([\s\S]*?)$/)
                        if (stepsMatch) {
                          return { section: 'Implementation Guide', text: 'Creating step-by-step implementation guide...' }
                        }
                      }
                      
                      // Check for testing checklist
                      if (content.includes('"testingChecklist"') && content.includes('[')) {
                        return { section: 'Testing Checklist', text: 'Preparing accessibility testing checklist...' }
                      }
                      ...
                      for (const { field, label, pattern } of fieldPatterns) {
                        ...
                        if (fieldStart !== -1) {
                          const afterField = content.substring(fieldStart + field.length + 4)
                          const quoteIndex = afterField.indexOf('"')
                          if (quoteIndex === -1) {
                            // Still writing this field
                            const rawText = afterField.replace(/\\n/g, '\n').replace(/\\"/g, '"')
                            return { section: label, text: rawText }
                          }
                        }
                      }
                      
                      return { section: 'Initializing Analysis', text: 'Connecting to accessibility expert AI...' }
                    }
                      ...
                      if (jsonMatch) {
                        const extractCompletedFields = (content: string) => {
                          ...
                          const completedFields = [
                            'issueTitle', 'severity', 'actualResult', 'expectedResult', 
                            'userImpact', 'recommendations', 'codeExample'
                          ]
                          
                          completedFields.forEach(field => {
                            const value = extractField(field, content)
                            if (value && content.includes(`"${field}":"${value}"`)) {
                              (partial as any)[field] = value
                            }
                          })
                          
                          // Handle severity validation
                          if (partial.severity && !['critical', 'high', 'medium', 'low'].includes(partial.severity)) {
                ...
                case 'error':
                  if (data.code === 'INSUFFICIENT_CREDITS') {
                    ...
                    setError(`${data.error} You've used all your trial analyses for this tool. Sign up to get 100 free credits!`)
                  } else {
                    setError(data.error)
                  }
                  setIsStreaming(false)
      ...
      try {
        ...
        const fallbackResponse = await fetch("/api/analyze-accessibility-issue", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (!fallbackResponse.ok) {
          if (fallbackData.code === 'INSUFFICIENT_CREDITS') {
            setError(`${fallbackData.error} Visit your dashboard to purchase more credits.`)
          } else if (fallbackData.code === 'TRIAL_LIMIT_EXCEEDED') {
            setError(`${fallbackData.error} You've used all your trial analyses for this tool. Sign up to get 100 free credits!`)
          } else {
            throw new Error(fallbackData.error || "Failed to analyze accessibility issue")
          }
          return
        }
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4 text-sm">
                ...
                <Link href="/unlimitedaccess">
                  <Button variant="ghost" size="sm" className="text-white hover:text-green-600 hover:bg-white/20">
                    Manage Access
                  </Button>
                </Link>
              </div>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl md:text-6xl font-bold text-slate-900 dark:text-white mb-6">
                AI Accessibility{" "}
                <span className="text-blue-600 dark:text-blue-400">Audit Helper</span>
              </h1>
              
              <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-400 mb-8 leading-relaxed">
                Get expert accessibility analysis with AI-powered insights and WCAG compliance recommendations
              </p>
            </div>
            ...
            <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm">
                ...
                <CardHeader className="pb-6">
                  <div className="text-center mb-6">
                    ...
                    <CardDescription className="text-lg text-slate-600 dark:text-slate-400">
                      Provide details about your accessibility challenge and get expert AI-powered analysis
                    </CardDescription>
                  </div>
                  ...
                  {!isSignedIn ? (
                      ...
                      <AlertDescription className="text-blue-700 dark:text-blue-300">
                        ...
                        {result && result.trialStatus && (
                          <div className="mt-2 text-sm">
                            Trial uses: {result.trialStatus.usageCount}/2 • {result.trialStatus.remainingUses} remaining
                          </div>
                        )}
                      </AlertDescription>
                    </Alert>
                    ...
                    <div className="flex items-center justify-center gap-4 p-4 bg-white/50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-slate-900 dark:text-white">Cost: 1 credit per analysis</span>
                      </div>
                      {result && (
              ...
              <CardContent className="p-8">
                <form ref={formRef} onSubmit={handleAnalyze} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="issue-description" className="text-lg font-semibold text-slate-900 dark:text-white">
                          Issue Description *
                        </Label>
                                              <Textarea
                      ...
                      {/* Tech Stack */}
                      <div className="space-y-3">
                        <Label htmlFor="tech-stack" className="text-lg font-semibold text-slate-900 dark:text-white">
                          Tech Stack (Optional)
                        </Label>
                                              <select
                        id="tech-stack"
                    ...
                    <div className="space-y-6">
                      {/* Code Snippet */}
                      <div className="space-y-3">
                        <Label htmlFor="code-snippet" className="text-lg font-semibold text-slate-900 dark:text-white">
                      ...
                      {/* Component Type */}
                      <div className="space-y-3">
                        <Label htmlFor="component-type" className="text-lg font-semibold text-slate-900 dark:text-white">
                      ...
                      {unlimitedAccess && (
                        <div className="space-y-3">
                          <Label htmlFor="ai-model" className="text-lg font-semibold text-slate-900 dark:text-white">
                            AI Model Selection
                          ...
                          >
                            {AVAILABLE_MODELS.map((model) => (
                              <option key={model.id} value={model.id}>
                                {model.recommended ? '◆ ' : ''}{model.name} - {model.cost} - {model.performance}
                              </option>
                            ))}
                          </select>
                          <div className="text-sm text-slate-600 dark:text-slate-400">
                            💡 <strong>Recommended models (◆)</strong> offer the best balance of quality and cost. 
                            <br/>• <strong>DeepSeek Chat V3.1 & R1</strong>: Best value for money, latest models
                            <br/>• <strong>Claude models</strong>: Best for detailed accessibility analysis
                            <br/>• <strong>Gemini models</strong>: Multimodal capabilities with good performance
                            <br/>• <strong>Meta Llama</strong>: Open-source excellence
                            <br/>• <strong>GPT-4o Mini</strong>: Good balance of speed and quality
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex gap-4 justify-center">
                    ...
                    >
                      {isAnalyzing ? (
                        <>
                          ...
                          {isStreaming ? currentStatus || "Analyzing..." : "Analyzing..."}
                        </>
                      ) : (
                        <>
                    ...
                    {(result || issueDescription) && (
                      <Button type="button" variant="outline" size="lg" onClick={clearForm} className="px-8 py-4 text-lg">
                        Clear Form
                      </Button>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              <div className="text-center">
                ...
                <h3 className="font-semibold text-slate-900 dark:text-white mb-2">Expert AI Analysis</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Advanced AI trained on WCAG guidelines</p>
              </div>
              ...
              <div className="text-center">
                ...
                <h3 className="font-semibold text-slate-900 dark:text-white mb-2">Code Solutions</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Practical code examples and guides</p>
              </div>
              ...
              <div className="text-center">
                ...
                <h3 className="font-semibold text-slate-900 dark:text-white mb-2">WCAG Compliance</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">Meet WCAG 2.2 Level AA standards</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <Card className="border-2 border-blue-200 dark:border-blue-700 shadow-lg">
                <CardHeader>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <CardTitle className="text-xl text-slate-900 dark:text-white">AI Accessibility Expert is Analyzing</CardTitle>
                      <CardDescription className="text-slate-600 dark:text-slate-400">{currentStatus}</CardDescription>
                    </div>
                  </div>
                </CardHeader>
              </Card>
                  ...
                  <CardContent>
                    <div className="p-4 bg-white/70 dark:bg-slate-800/70 rounded-lg border border-green-200 dark:border-green-700">
                      <p className="text-green-900 dark:text-green-100 leading-relaxed whitespace-pre-wrap">
                        {typingText}<span className="animate-pulse text-green-600 font-bold">|</span>
                      </p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div className="space-y-4">
                  ...
                  {partialResult.issueTitle && (
                    <Card className="border-green-200 bg-green-50/30 dark:border-green-700 dark:bg-green-950/20">
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <CheckCircle className="h-4 w-4 text-green-600" />
                              <span className="text-sm font-medium text-green-800 dark:text-green-200">✅ Issue Identified</span>
                            </div>
                            <CardTitle className="text-lg text-slate-900 dark:text-white">{partialResult.issueTitle}</CardTitle>
                          ...
                          {partialResult.severity && (
                            <Badge className={`${SEVERITY_CONFIG[partialResult.severity].color} border`}>
                      ...
                      {(partialResult.actualResult || partialResult.expectedResult) && (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {partialResult.actualResult && (
                              <div className="p-3 bg-red-50 dark:bg-red-950/20 rounded-lg border border-red-200 dark:border-red-800">
                                ...
                                <p className="text-sm text-red-800 dark:text-red-200">{partialResult.actualResult}</p>
                              </div>
                            )}
                            {partialResult.expectedResult && (
                              <div className="p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
                                ...
                                <p className="text-sm text-green-800 dark:text-green-200">{partialResult.expectedResult}</p>
                              </div>
                            )}
                          </div>
                        </CardContent>
                  ...
                  {partialResult.recommendations && (
                    <Card className="border-green-200 bg-green-50/30 dark:border-green-700 dark:bg-green-950/20">
                      <CardHeader>
                        <div className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                          <span className="text-sm font-medium text-green-800 dark:text-green-200">✅ Recommendations Ready</span>
                        </div>
                  ...
                  {partialResult.codeExample && (
                    <Card className="border-green-200 bg-green-50/30 dark:border-green-700 dark:bg-green-950/20">
                      <CardHeader>
                        <div className="flex items-center gap-2">
                          <CheckCircle className="h-4 w-4 text-green-600" />
                          <span className="text-sm font-medium text-green-800 dark:text-green-200">✅ Code Solution Ready</span>
                        </div>
                        <CardTitle className="text-lg text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <div className="space-y-8">
              ...
              <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div>
                      <CardTitle className="text-xl md:text-2xl text-slate-900 dark:text-white mb-2">{result.issueTitle}</CardTitle>
                      <CardDescription className="text-lg text-slate-600 dark:text-slate-400">{result.userImpact}</CardDescription>
                    </div>
                    <Badge className={`${SEVERITY_CONFIG[result.severity].color} border text-lg px-3 py-1`}>
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-4 bg-red-50 dark:bg-red-950/20 rounded-lg border border-red-200 dark:border-red-800">
                      ...
                      <p className="text-red-800 dark:text-red-200">{result.actualResult}</p>
                    </div>
                    <div className="p-4 bg-green-50 dark:bg-green-950/20 rounded-lg border border-green-200 dark:border-green-800">
                      ...
                      <p className="text-green-800 dark:text-green-200">{result.expectedResult}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                <CardHeader>
                  <CardTitle className="text-xl text-slate-900 dark:text-white">WCAG Compliance Requirements</CardTitle>
                  <CardDescription>Relevant WCAG criteria that apply to this accessibility issue</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid gap-4">
                    {result.wcagCriteria.map((criterion, index) => (
                      <div key={index} className="p-4 rounded-lg border bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                        <div className="flex items-center justify-between mb-3">
                          <span className="font-semibold text-slate-900 dark:text-white">{criterion.criterion}</span>
                          <Badge variant="outline" className="text-sm">Level {criterion.level}</Badge>
                        </div>
                        <p className="text-slate-600 dark:text-slate-400">{criterion.description}</p>
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                <CardHeader>
                  ...
                  <CardDescription>Detailed guidance on how to fix this accessibility issue</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="prose prose-slate dark:prose-invert max-w-none">
                    <p className="text-slate-700 dark:text-slate-300 leading-relaxed text-lg">{result.recommendations}</p>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => copyToClipboard(result.codeExample!, "Code")}
                      >
                        {copied === "Code" ? <CheckCircle className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                        {copied === "Code" ? "Copied!" : "Copy Code"}
                      </Button>
                    </div>
                    <CardDescription>Improved code example implementing the accessibility fix</CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                <CardHeader>
                  <CardTitle className="text-xl text-slate-900 dark:text-white">Implementation Steps</CardTitle>
                  <CardDescription>Follow these steps to implement the accessibility fix</CardDescription>
                </CardHeader>
                <CardContent>
                  <ol className="space-y-4">
                    {result.implementationSteps.map((step, index) => (
                      <li key={index} className="flex gap-4">
                        ...
                        <span className="text-slate-700 dark:text-slate-300 leading-relaxed pt-1">{step}</span>
                      </li>
                    ))}
                  </ol>
                </CardContent>
              ...
              <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                <CardHeader>
                  <CardTitle className="text-xl text-slate-900 dark:text-white">Testing Checklist</CardTitle>
                  <CardDescription>Verify your implementation with these accessibility tests</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {result.testingChecklist.map((test, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
              ...
              {result.relatedResources && result.relatedResources.length > 0 && (
                <Card className="border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                  <CardHeader>
                    <CardTitle className="text-xl text-slate-900 dark:text-white">Related Resources</CardTitle>
                    <CardDescription>Additional reading and documentation</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="grid gap-3">
                      {result.relatedResources.map((resource, index) => (
                        ...
                        >
                          <span className="font-medium text-slate-900 dark:text-white">{resource.title}</span>
          ...
          {!result && !isAnalyzing && !isStreaming && (
            <Card className="border-dashed border-2 border-slate-300 dark:border-slate-600">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <h3 className="text-2xl font-semibold text-slate-900 dark:text-white mb-3">Ready to Analyze</h3>
                <p className="text-lg text-slate-600 dark:text-slate-400 mb-6 max-w-md">
                  Describe your accessibility issue above and get expert AI analysis with detailed recommendations.
                </p>
                <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
                  <Coins className="h-5 w-5" />
                  <span className="font-medium">{isSignedIn ? "1 credit per analysis" : "2 free trials available"}</span>
                </div>
              </CardContent>
            </Card>
          )}
