...
export default function HeadingAnalyzer() {
  ...
  const analyzeHeadings = (html: string): AnalysisResult => {
    ...
    if (h1Count === 0) {
      issues.push({
        ...
        recommendation: 'Add a clear page-level heading so users can identify the main topic. WCAG does not require a specific H1 count.'
      })
    } else if (h1Count > 1) {
      issues.push({
        type: 'info',
        message: `Multiple H1 tags found (${h1Count})`,
        recommendation: 'Multiple H1 elements are valid HTML, but one clear page title is usually easier for users to understand. Review the outline in context.'
      })
    }
    ...
    longHeadings.forEach(heading => {
      issues.push({
        ...
        heading,
        recommendation: 'Review whether the heading can be shorter without losing meaning. This is a readability heuristic, not a WCAG requirement.'
      })
    })
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (url) {
        // URL fetching stays first-party and applies server-side private-network checks.
        try {
          ...
          if (!response.ok) throw new Error(data.error || "Unable to fetch URL")
          htmlToAnalyze = data.html
        } catch (fetchError) {
          setError(fetchError instanceof Error ? fetchError.message : "Unable to fetch URL. Try entering the HTML directly.")
...
Structure heuristic: ${results.totalScore}/100
...
Report generated by Accessibility.build
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="sr-only">Heading analysis workspace</h2>
      <Card className="mb-8">
        <CardHeader>
        ...
        <CardContent>
          <Tabs defaultValue="url" className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="url" className="text-slate-700 dark:text-slate-200">Analyze URL</TabsTrigger>
              <TabsTrigger value="html" className="text-slate-700 dark:text-slate-200">Analyze HTML</TabsTrigger>
            </TabsList>
          ...
          <Card>
            <CardHeader>
              <div className="flex flex-col md:!flex-row md:items-center justify-between gap-4 md:gap-0">
                ...
                <Button variant="outline" onClick={copyResults} className="w-full md:w-auto self-start md:self-auto">
                  {copied ? (
                    <>
            ...
            <CardContent>
              {results.issues.length > 0 ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="font-medium text-green-600">No structural heuristics were triggered.</p>
                  <p>Review heading meaning and page context manually before treating the outline as complete.</p>
                </div>
              )}
            </CardContent>
          </Card>
