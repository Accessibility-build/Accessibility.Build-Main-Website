...
export function exportTypography(format: ExportFormat, input: ExportInput): ExportFile {
  switch (format) {
    case "css":
    ...
    case "json":
      return {
        filename: `${kebab(input.name)}.json`,
        language: "json",
        content: JSON.stringify({ name: input.name, tokens: input.tokens }, null, 2),
      }
  }
}

function kebab(s: string): string {
  return s.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "typography"
}
